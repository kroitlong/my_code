...
TEST(LRUReplacerTest, SampleTest) {
    ...
    lru_replacer.unpin(1);
    // for (auto it = lru_replacer.LRUlist_.begin(); it != lru_replacer.LRUlist_.end(); ++it) {
    //     std::cout << *it << " ";
    // }
    // std::cout << std::endl;
...
TEST_F(BufferPoolManagerTest, SampleTest) {
    ...
    for (size_t i = 1; i < buffer_pool_size; ++i) {
        ...
        //std::cout<<"pin_count:"<<bpm->new_page(&page_id_temp)->pin_count_<<std::endl;
    }
    ...
    bpm->unpin_page(PageId{fd, 14}, false);
    page0 = bpm->fetch_page(PageId{fd, 0});
    //std::cout<<"yes3"<<std::endl;
    EXPECT_EQ(0, strcmp(page0->get_data(), "Hello"));
...
TEST_F(BufferPoolManagerConcurrencyTest, ConcurrencyTest) {
    ...
    for (int run = 0; run < num_runs; run++) {
        //debug
        // std::cout << "runtimes:" << run << std::endl;
        ...
        for (int tid = 0; tid < num_threads; tid++) {
            threads.push_back(std::thread([&bpm, fd, tid]() { // NOLINT
