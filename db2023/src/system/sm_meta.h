...
struct TabMeta {
    ...
    bool is_index(const std::vector<std::string> &col_names_) const {
        for (auto &index : indexes) {
            if (index.col_num == col_names_.size()) {
                size_t i = 0;
                for (; i < index.col_num; ++i) {
                    if (index.cols[i].name.compare(col_names_[i]) != 0)
                        break;
                }
                if (i == index.col_num) {
                    return true;
...
public:
    ...
    friend std::ostream &operator<<(std::ostream &os, const DbMeta &db_meta) {
        os << db_meta.name_ << '\n' << db_meta.tabs_.size() << '\n';
        //std::cout << db_meta.tabs_.size() << std::endl;
