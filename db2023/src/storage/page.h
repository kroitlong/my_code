...
public:
    ...
    inline char *get_data() {
        //debug
        // if (data_ != "") {
        //     std::string tostring = data_;
        //     encoded = encodeString(tostring);
        //     printf("data_: ");
        //     for (int num : encoded) {
        //         printf("%d ", num);
        //     }
    ...
    std::vector<int> encodeString(const std::string &str) {
        std::vector<int> encoded;
        for (char c : str) {
...
private:
    void reset_memory() {
        memset(data_, OFFSET_PAGE_START, PAGE_SIZE);    // 将data_的PAGE_SIZE个字节填充为0
    }
