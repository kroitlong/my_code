...
private:
    ...
    static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
        ...
        if (auto x = std::dynamic_pointer_cast<Help>(node)) {
            ...
            std::cout << "STRING_LIT\n";
            print_val(x->val, offset);
        } else if (auto x = std::dynamic_pointer_cast<datetime>(node)) {
            std::cout << "DATETIME\n";
            for (auto i = 0; i < 8; i++) {
                print_val(static_cast<int>(x->time[i]), offset);
            }
        } else if (auto x = std::dynamic_pointer_cast<bigint>(node)) {
            std::cout << "BIGINT\n";
            print_val(x->val, offset);
            print_val(x->is_legal, offset);
