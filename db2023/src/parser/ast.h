...
enum SvType {
    SV_TYPE_INT, SV_TYPE_FLOAT, SV_TYPE_STRING, SV_TYPE_DATETIME, SV_TYPE_BIGINT
};
...
struct datetime : public Value {
    int8_t time[8];

    datetime(int8_t year_h, int8_t year_l, int8_t month_, int8_t day_, int8_t hour_, int8_t minute_, int8_t second_)
        : time{year_h, year_l, month_, day_, hour_, minute_, second_, 1} {}
...
struct bigint : public Value {
    int64_t val;
    bool is_legal;
    bigint(int64_t val_): val(val_), is_legal(1) {}
    bigint() {
        val = 0;
        is_legal = 0;
    }
